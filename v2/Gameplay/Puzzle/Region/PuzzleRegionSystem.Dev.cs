...
{
    public partial class PuzzleRegionSystem
    {
        ...
        private void Dev_TransitionByScore(int curRegionIndex)
        {
            ...
            var region = dev_curRegionIndex == 0 ? progression.InitialPuzzle : entries[dev_curRegionIndex - 1].puzzleRegion;
            _transitionRoutine = StartCoroutine(TransitionTo(region));
        }

        [ButtonGroup("Dev/Transition")]
        [Button(Expanded = true), DisableInEditorMode]
        private void Dev_TransitionToPrev() => Dev_TransitionByScore(dev_curRegionIndex - 1);

        [ButtonGroup("Dev/Transition")]
        [Button(Expanded = true), DisableInEditorMode]
        private void Dev_TransitionToNext() => Dev_TransitionByScore(dev_curRegionIndex + 1);
    }
}
#endif
